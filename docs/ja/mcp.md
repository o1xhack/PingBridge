...
Agent は実 notification を送る前に `preview` / dry-run style tests で設定を検証してください。provider secrets を MCP config に入れないでください。MCP が必要とするのは PingBridge endpoint と app token だけです。
