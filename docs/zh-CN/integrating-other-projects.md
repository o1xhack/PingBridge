...
该测试会创建临时外部项目、安装 packed `@o1x/pingbridge-client` tarball、启动本地 PingBridge HTTP server，调用 `health`、`checkConfig`、`previewMessage`、`sendMessage`，并验证 preview 不发送而 send 会调用 fake provider。
